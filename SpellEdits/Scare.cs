    ...
    partial class SpellEdits
    {
        public static void EditScare()
        {
            ...
            try
            {
                ...
                var scareTargetAction = onRun.Actions.Actions.FirstOfType<Conditional>();
                var oldConditions = scareTargetAction.ConditionsChecker.Conditions;
                var newConditions = oldConditions.Where(cond => cond as ContextConditionHitDice == null)
                    .ToArray(); // reversed conditon
                scareTargetAction.ConditionsChecker.Conditions = newConditions;
                Main.Logger.Log($"Successfully installed Scare patch!");
            }
            catch (Exception e)
            {
